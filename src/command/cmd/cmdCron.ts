...
export const cron = new JsCron({ timezone: 'America/Bogota', runOnInit: true })
export const tasks = new Map<string, any>()

function isTooFrequent(field: string, minVal: number) {
  if (field === '*') return true
  const m = field.match(/^\*\/(\d+)$/)
  if (m && parseInt(m[1], 10) < minVal) return true
  return false
}

function validateCron(exp: string) {
...
function parseArgs(raw: string) {
  ...
  const labels: Record<string, string> = { expression: '-e', url: '-url', type: '-t', returnPath: '-r', message: '-m' }
...
export default {
  ...
  async cmd(bot: any, msg: any, match: any) {
    ...
    const parsed: any = parseArgs(input)
    ...
    const callback = async () => {
      try {
        const res = await globalThis.fetch(parsed.url)
        if (parsed.type === 'json') {
          const json = await res.json()
          let text: string
          if (parsed.message) {
            text = (tmpl as any)(parsed.message, json)
          } else {
            text = String(parsed.returnPath.split('.').reduce((a: any, b: string) => a?.[b], json))
          }
