import { searchTracks, searchArtists, searchAlbums, searchPlaylists, searchVideos, getCoverUrl, downloadTrack } from '../../services/tidal-instances.service.js'

const SEARCH_TYPES: Record<string, { name: string; title: string; searchFn: Function; type: string; slice: number }> = {
...
export default async (bot: any, msg: any) => {
  ...
  try {
    ...
    const results = await Promise.all(items.slice(0, slice).map(async (item: any) => {
