...
async function cmd (bot, msg, args) {
  ...
  const match = input.match(/^[.q]([qsapv]|al)?(?:\s+(.*))?$/i) || input.match(/^([qsapv]|al)?(?:\s+(.*))?$/i)
  if (match) {
...
async function downloadById(bot, chatId, trackId) {
  try {
    ...
    bot.sendMessage(chatId, '⏬ Descargando (FLAC)...')
...
module.exports = {
  active: true,
  ExpReg: new RegExp(`^/tidal(?:@${configEnv.USERNAME_BOT})?\\s+(.+)$`, 'im'),
  cmd,
  sendPage
}