...
const { cron: cronScheduler, tasks: cronTasks } = require('../../command/cmd/cmdCron.cjs')
  ...
  switch (action) {
    ...
    case 'cron_test': {
      ...
      try {
        ...
        await client.sendMessage(parseInt(targetChatId), `❌ Error en test *${name}*: ${err.message}`, { parse_mode: 'Markdown' })
      }
      break
    }
    case 'cron_del': {
